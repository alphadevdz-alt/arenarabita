...
export function menusFor(role: string): { id: string; label: string }[] {
  ...
  if (role === 'SYSTEM_ADMINISTRATOR') return [{ id: 'home', label: 'لوحة المشرف' }, verify, { id: 'competitions', label: 'المنافسات' }, { id: 'participants', label: 'قوائم التلاميذ' }, { id: 'institutions', label: 'المؤسسات' }, { id: 'cards', label: 'بطاقات الانخراط' }, { id: 'users', label: 'المستخدمون' }, { id: 'organizations', label: 'الرابطات' }, { id: 'audit', label: 'التدقيق' }, { id: 'reports', label: 'التقارير' }, account];
  if (role === 'NATIONAL_ADMINISTRATOR') return [{ id: 'home', label: 'لوحة وطنية' }, verify, { id: 'seasons', label: 'المواسم' }, { id: 'competitions', label: 'المنافسات' }, { id: 'participants', label: 'قوائم التلاميذ' }, { id: 'cards', label: 'بطاقات الانخراط' }, { id: 'announcements', label: 'الإعلانات' }, { id: 'results', label: 'النتائج' }, { id: 'reports', label: 'التقارير' }, account];
  if (role === 'ASSOCIATION_ADMINISTRATOR') return [{ id: 'home', label: 'لوحة الرابطة' }, verify, { id: 'competitions', label: 'المنافسات' }, { id: 'participants', label: 'قوائم التلاميذ' }, { id: 'approvals', label: 'طلبات الانخراط' }, { id: 'cards', label: 'بطاقات الانخراط' }, { id: 'institutions', label: 'المؤسسات' }, { id: 'licenses', label: 'التراخيص' }, { id: 'entries', label: 'التسجيلات' }, account];
  if (role === 'ASSOCIATION_REPRESENTATIVE') return [{ id: 'home', label: 'لوحة التمثيل' }, verify, { id: 'competitions', label: 'المنافسات' }, { id: 'participants', label: 'قوائم التلاميذ' }, { id: 'entries', label: 'التسجيلات' }, account];
  if (role === 'DAIRA_OFFICER') return [{ id: 'home', label: 'لوحة الدائرة' }, verify, { id: 'competitions', label: 'المنافسات' }, { id: 'institutions', label: 'مؤسسات الدائرة' }, { id: 'participants', label: 'قوائم التلاميذ' }, account];
  return [{ id: 'home', label: 'لوحة المؤسسة' }, verify, { id: 'participants', label: 'قائمة التلاميذ' }, { id: 'competitions', label: 'المنافسات' }, { id: 'cards', label: 'بطاقات الانخراط' }, { id: 'licenses', label: 'طلب ترخيص' }, { id: 'entries', label: 'التسجيل في منافسة' }, account];
}
...
export function RoleHome({ token, role, user }: { token: string; role: string; user: any }) {
  ...
  const rights: Record<string, string[]> = {
    ...
    ASSOCIATION_ADMINISTRATOR: ['موافقة/رفض انخراط المؤسسات', 'تأكيد ملفات التلاميذ', 'إصدار البطاقات والتراخيص'],
    ASSOCIATION_REPRESENTATIVE: ['عرض المشاركين', 'متابعة التسجيلات', 'بدون تسجيل تلاميذ'],
    DAIRA_OFFICER: ['عرض مؤسسات الدائرة', 'عرض المشاركين في النطاق', 'بدون تسجيل تلاميذ'],
    MEMBER_INSTITUTION_USER: ['تسجيل تلاميذ المؤسسة فقط', 'طلب انخراط في منافسة (بانتظار تأكيد الرابطة)', 'طلب ترخيص']
  };
  return (
