...
export function buildApp() {
  ...
  app.get('/api/v1/public/geography/wilayas/:id/dairas', async (request, reply) => { const id=z.coerce.number().int().min(1).max(69).safeParse((request.params as {id:string}).id); if(!id.success)return reply.code(400).send({error:'validation_error'}); const { pool } = await import('./infrastructure/db.js'); const result = await pool.query('SELECT id,wilaya_id,name,ar_name FROM dairas WHERE wilaya_id=$1 ORDER BY name',[id.data]); return { data: result.rows }; });
    app.get('/api/v1/public/geography/schools', async (request) => { const query=z.object({wilayaId:z.coerce.number().int().min(1).max(69).optional(),cycle:z.string().max(40).optional(),q:z.string().max(80).optional(),page:z.coerce.number().int().min(1).default(1),pageSize:z.coerce.number().int().min(1).max(50).default(25)}).parse(request.query); const values=[]; const where=['1=1']; if(query.wilayaId){values.push(query.wilayaId);where.push(`wilaya_id=$${values.length}`)} if(query.cycle){values.push(query.cycle);where.push(`cycle=$${values.length}`)} if(query.q){values.push(`%${query.q}%`);where.push(`(name ILIKE $${values.length} OR name_ar ILIKE $${values.length} OR commune_name ILIKE $${values.length})`)} const offset=(query.page-1)*query.pageSize; values.push(query.pageSize,offset); const { pool } = await import('./infrastructure/db.js'); const result=await pool.query(`SELECT id,name,name_ar,name_fr,wilaya_id,commune_name,cycle,kind FROM school_directory WHERE ${where.join(' AND ')} ORDER BY name NULLS LAST LIMIT $${values.length-1} OFFSET $${values.length}`,values); return {data:result.rows,page:query.page,pageSize:query.pageSize}; });
